...
def test_folder_project_rejects_overlap_with_managed_workspace(tmp_path, monkeypatch):
    _reset_studio_db(tmp_path, monkeypatch)
    managed = studio_db.upsert_chat_project(_project("managed-project"))
    nested = Path(managed["rootPath"]) / "nested"
    nested.mkdir()

    with pytest.raises(studio_db.ProjectWorkspaceError, match = "overlaps"):
